...
export const projects: Project[] = [
...

  {
    ...
    features: [
      ...
      "Admin dashboard for managing games, products, orders, reports, announcements, FAQ, about content, settings, templates, live chat, analytics, and developer tool.",
...

  {
    ...
    title: "FlowCanvas",
    shortDesc:
      "A local-first visual workflow & diagram editor that runs entirely in the browser — hand-rolled infinite canvas, 64 node types across 8 categories, a deterministic step-through simulation engine, live validation, presentation mode, and JSON/SVG/PNG export with zero backend.",
    longDesc:
      "FlowCanvas is a local-first visual workflow and diagram editor built on the principle that a diagramming tool should run entirely in the browser with zero backend, zero accounts, and zero data leaving the device. Every document — nodes, shapes, text, sticky notes, frames, edges, and comment threads — persists to IndexedDB, alongside snapshots and application settings in the same local store. The editor is built around a hand-rolled infinite canvas engine using Pointer Events with pointer capture, zoom-at-pointer, grid snapping, smart alignment guides, marquee selection, rotation and resize handles, a click-to-jump minimap, and HTML5 drag-and-drop from the node palette. The node system ships 64 node types across 8 categories (general, flow, automation, developer, data, product, state, architecture) with per-category accent colors, port rules, and typed default data. Edges render in three routing modes — straight, elbow, curved — with labels, dashed variants, and live valid/invalid connection previews while dragging. Ten starter templates, from basic workflows and approval chains to CI/CD pipelines, state machines, microservice architectures, and incident response maps, are real fully-editable documents rather than static previews. A deterministic simulation engine steps through the graph with start/pause/step/replay controls, a live variables panel, structured condition operators evaluated without eval(), and a 500-step guard against runaway loops. A pure-function validation engine detects dangling edges, orphan nodes, missing start/end markers, cycles, disconnected components, and overloaded ports, surfaced with severity colors in a dedicated Issues tab. The editor also includes a Ctrl+K command palette covering every action, a frame-based presentation mode with keyboard navigation and fullscreen, draft recovery on unclean exits, a four-tab right panel (Inspect, Layers, Issues, Comments), and exports to JSON, SVG, and PNG — all drawn from real document data rather than screenshots. The whole interface speaks Indonesian and follows a 'Drafting Table' design language: warm paper tones, burnt sienna accents, technical mono readouts, and a fully token-based light/dark theme system, with semantic versioning visible in Settings → Tentang.",
    images: [],
    techStack: [
      "Next.js 16",
      "React 19",
    ...
    category: "web",
    status: "live",
    liveUrl: "https://flowcanvas-editor.vercel.app/",
    githubUrl: "https://github.com/neuralforgeio/FlowCanvas",
    features: [
      "Local-first architecture — every document, snapshot, and setting lives in IndexedDB; no backend, no accounts, no network calls, data never leaves the browser",
      "Hand-rolled infinite canvas engine — Pointer Events with setPointerCapture, zoom-at-pointer, grid snap, smart alignment guides, marquee selection, rotate/resize handles, and a click-to-jump minimap",
      "64 node types across 8 categories (Umum, Alur, Otomasi, Developer, Data, Produk, State, Arsitektur) with per-category accents, port rules, and typed default data",
      "Edge system with three routing modes — straight, elbow, curved — plus labels, dashed variants, and live valid/invalid connection previews while dragging",
      "Deterministic workflow simulation — start/pause/step/replay controls, progress bar, status badges, and a variables panel; conditions evaluated through structured operators with zero eval() and a 500-step runaway guard",
      "Pure-function validation engine detecting dangling edges, orphan nodes, missing start/end markers, cycles, disconnected components, and overloaded ports — surfaced with severity colors in a dedicated Issues tab",
      "Ten real, fully editable starter templates: basic workflow, approval chain, API request, CI/CD, data pipeline, user journey, state machine, microservice, and incident response",
      "Undo/redo built on the command pattern with immutable before/after snapshots and gesture coalescing — one drag, slider sweep, or typing burst collapses into a single history entry",
      "Ctrl+K command palette covering every editor action: add any object from the registry, edit, view, flow/simulation, and file operations",
      "Frame-based presentation mode — frames become slides navigable by keyboard, on-screen pill, or fullscreen",
      "Four-tab right panel: Inspect (typed property fields), Layers (z-order object list), Issues (validation results), and Comments (threaded discussion pins)",
      "Export to JSON, SVG, and PNG — all rendered from document data itself, not screenshots — plus SVG thumbnails for the document hub",
      "Draft recovery — snapshots detect unclean exits and offer restore/discard on next launch; autosave debounced at 800ms",
      "'Drafting Table' design system: fully token-based light/dark theming, warm paper surfaces, burnt sienna accent, technical mono readouts, Indonesian UI, and visible semantic versioning in Settings → Tentang",
    ],
    challenges: [
      {
        title: "A design system that broke in the dark",
        description:
          "The first release hardcoded light-theme values directly into canvas objects — nodes rendered blinding white in dark mode and sticky notes became nearly unreadable. The fix was a full v1.1.0 rework: every canvas color migrated to CSS tokens (--node, --node-stroke, --note, --note-ink…), the internal JavaScript dark prop was deleted entirely so theming flows purely through CSS, and SVG/PNG exports and hub thumbnails were realigned to the new node structure. The rule that emerged: if a component needs to know the theme in JavaScript, the token layer has failed.",
      },
      {
        title: "Undo history that drowned in gestures",
        description:
          "The command-pattern history stores immutable before/after snapshots, but continuous interactions — dragging a node, sweeping a slider, typing a label — each fire dozens of mutations. Recorded naively, one drag produced fifty undo entries and Ctrl+Z became useless. The solution wraps continuous gestures in beginGesture/endGesture with a 500ms debounce so an entire interaction collapses into a single coalesced history entry, while discrete actions (delete, reorder, route change) still record individually.",
      },
      {
        title: "Simulating workflows without eval()",
        description:
          "The simulation engine evaluates condition branches (variable, operator, value) against a live variable map. The tempting shortcut is evaluating the expression as code — an eval() injection hole in a tool that also imports untrusted JSON documents. Instead, conditions are typed as a discriminated union of structured operators (equals, contains, greaterThan…), the stepper enforces MAX_SIMULATION_STEPS to guarantee termination, and imported documents are parsed with a zod schema that rejects malformed files wholesale — no partial imports, no silent field coercion.",
      },
      {
        title: "Exporting PNG without a headless browser",
        description:
          "PNG export on static hosting usually means puppeteer or a screenshot service — both contradict the zero-backend principle. FlowCanvas instead builds a complete SVG string from the document data itself (nodes, edges, labels, notes, frames — not a DOM scrape), loads it into an Image, and rasterizes onto a canvas for download. The same document-data-to-SVG pipeline powers hub thumbnails and SVG export, so what the user previews is exactly what gets exported.",
      },
    ],
    startDate: "2026-09",
    endDate: undefined,
